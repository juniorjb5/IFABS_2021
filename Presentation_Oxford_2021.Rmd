...
.font120[
The number of optimal factors $\hat{k}$ is obtained from the estimation of the corresponding factor loadings that accompany the observed factors, which can be consistently estimated from $\hat{k}=\arg\min_{0\le k\le kmax}PC(k)$ with $r\le kmax$. Let:
...
$$V\left(k,{\hat{F}}^k\right) = \min_{\Lambda}{\frac{1}{NT}}\sum_{i=1}^{N}\sum_{t=1}^{T}\left(X_{it}-\lambda_i^{k^\prime}\hat{F}_t^k\right)^2$$
