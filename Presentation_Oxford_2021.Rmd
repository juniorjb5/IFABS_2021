...
.font120[
The number of optimal factors $\hat{k}$ is obtained from the estimation of the corresponding factor loadings that accompany the observed factors, which can be consistently estimated from $\hat{k}=\arg{{min}_{0\le k\le k m a x\ }PC(k)}$ with $r\le kmax$. Let:
...
<div>
$$
V\left(k,{\hat{F}}^k\right)=\min_{\Lambda}{\frac{1}{NT}}\sum_{i=1}^{N}\sum_{t=1}^{T}{{{(X}_{it}-\lambda_i^{k^\prime}{\hat{F}}_t^k)}^2}
$$
</div>
...
where `\\(M\\)` is a matrix of size `\\(T\times K\\)` with each row equal to `\\({(m}_1,\ \ldots,\ m_N)\\)`, i.e., the vector with the mean of each variable.
